...
class Application_Model_Parceltracking extends Zend_Custom
{
    public function parcelinformation(){
	   $select = $this->_db->select()
		...
		$this->getOurTracking();//echo "<pre>";print_r($this->getData);die;
		return $this->getData;
		//
	}
	...
	public function getOurTracking(){ //echo "<pre>";print_r($this->getData);die;
	   ...
	   $addedXY = $this->getLatLong($this->getData['sendercncode'],$this->getData['sender_city']);
	   //Added
	   ...
	   //Checkin
	    ...
	    if($this->getData['checkin_status']=='1'){
		    $checkin_detail = $this->getCustomerDetails($this->getData['checkin_by']);
		// echo "<pre>"; print_r($checkin_detail);die;
			$addedXYcheckin = $this->getLatLong($checkin_detail['cncode'],$checkin_detail['city']);
	   ...
	   //Hub check-in
	    if($this->getData['hub_status']=='1' && isset($checkin_detail['hub_country'])){
			...
			$addedXY = $this->getLatLong($checkin_detail['hub_country'],$checkin_detail['hub_city']);
			...
			if($hubdetails['hub_checkin_status']=='1'){
				$addedXY = $this->getLatLong($checkin_detail['hub_scan_country'],$checkin_detail['hub_scan_city']);
	   ...
	   //Electronic data Exchange
	    if($this->getData['edi_status']=='1' && empty($this->getData['scrapws'])){
		    //$checkin_detail = $this->getCustomerDetails($this->getData['checkin_by']);
			///$addedXY = $this->getLatLong($checkin_detail['cncode'],$checkin_detail['city']);
		...
		foreach($this->getData['scrapws'] as $key=>$scrappingData){
		   $addedXY = $this->getLatLong($scrappingData['country'],$scrappingData['city']);
		   if($this->getData['error_status']==1){
	...
	public function trackinglink(){
	  ...
	  case 51:
	   ...
	   break; 
		...
		case 64:
		   $this->getData['tracking_link']  = "http://www.bizcourier.eu/pegasus/courier04/search00.php?pcode=SEARCHVOUCHER&r18p01=".$this->getData[TRACENR];
	...
	public function getScrapData(){
	   ...
	   switch($this->getData[FORWARDER_ID]){
		  ...
		  case 4:
		   ...
		   break;
		   case 64:
		     $scrappingData =$this->BizCourierTracking($this->getData);
		   break;
		  default:
	...
	public function BizCourierTracking($Parcelinfo='')
	{


		if(isset($Parcelinfo['tracenr_barcode']) && !empty($Parcelinfo['tracenr_barcode'])){

			try {
   				
				$countrydetail = $this->getCountryDetail($this->getData['country_id']);
   				$client = new SoapClient( 'http://www.bizcourier.eu/pegasus_cloud_app/service_01/full_history.php?wsdl' , array('trace' => 1));
				$data = array(
			        'Voucher'=> $Parcelinfo['tracenr_barcode'],
			    ); 
   				$trackings = $client->__soapCall('full_status',$data);
				...
				// die;
   				...
   				if(!empty($trackings)){
   					...
   					foreach ($trackings as $key => $tracking) {
   						...
   						$values['updateon'] = date('Y-m-d H:i:s',strtotime($tracking->Status_Date." ".$tracking->Status_Time));
			        	$values['location'] = $this->getData['rec_city']."(".$countrydetail['cncode'].")";
			        	$values['status'] = $tracking->Status_Description_En;
			        	$values['country'] = $countrydetail['cncode'];
			        	$values['city'] = $this->getData['rec_city'];
			        	$rows[] = $values;
   					}
   				}
  ...
  public function DPDTATracetrack($data=array()){
	...
	$result = $client->GetTuDetail($reference);
	 if($result->ExitCode->ErrorCode==0){
 	   if(count($result->History)>1){ //echo "<pre>";print_r($result->History);die;
			...
			$trackcountry = $this->getCountryDetail($history->CountryName,3);
			$Data['country'] =  $trackcountry['cncode'];
			if(trim($history->LocationName)==''){
			      $countrydetail = $this->getCountryDetail($data['country_id']);
   ...
   public function  PostatTracking($parcelinfo=array()){
	    ...
	    /*$select = $this->_db->select()
								...
								//print_r($select->__toString());die;
	     $parceltrackdata =  $this->getAdapter()->fetchAll($select);
	  ...
	  if(!empty($trackinglist->parcel->trackingBarcode)){
			 ...
			 foreach($trackinglist->parcel->deliveryStatus->status as $status){
				 if($data['country_id']==9){
					...
					if($i==0){
						$depotAdd = $this->getCustomerDetails($data['parent_id']);
					    ...
					    $trackcountry = $this->getCountryDetail(utf8_encode($status->location),3);
		    ...
		    for($i=3; $i<count($explodebytr);$i++){
				...
				if($i<5 && commonfunction::trimString($Data['status'])!='In the delivery process' && commonfunction::trimString($Data['status'])!='Delivered'){
					$Data['country'] = "ES";
				 	...
				 	$country_detail = $this->getCountryDetail($data['country_id']);
    ...
    public function parcelErrorModification($data){ 
		...
		elseif(strlen(trim($this->getData['rec_phone']))<7){
			...
			if(empty($modifiedAddress)){
				...
				$this->_db->update(SHIPMENT,array_filter($updatearray),"shipment_id='".$this->getData[SHIPMENT_ID]."'");
				$this->_db->update(SHIPMENT_BARCODE,array('error_status'=>'0'),"barcode_id='".$this->getData[BARCODE_ID]."'");
				...
				if($this->getData['delivery_status']=='0' && $this->getData['forwarder_detail']['forwarder_email']!=''){
					...
					if($mailOBj->Send()) {
						$this->_db->update(WRONG_ADDRESS_MODIFICATION,array('email_sent_timestamp'=>new Zend_Db_Expr('NOW()')),"log_id='".$logID."'");
