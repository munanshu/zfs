...
class settings_Model_Statuscode extends Zend_Custom
{
    ...
    **/	 
     ...
     {
     	parent::init();
        $this->SettingModelObj = new settings_Model_Settings();
     }
  ...
  **/	  
    ...
    **/	 
     ...
     public function ConvertTextbyLang($text='',$langcode='',$changebackcode='')
     {	
     	...
     	//  $newLocale = new Zend_Locale();
     	 ...
     	 print Zend_Locale::getTranslation('fr', 'language', 'en');
     	 die;
	    $newLocale->setLocale($langcode);
		...
		$newtext = $translate->translate($text);

		echo $newtext;die;
		if(isset($changebackcode) && !empty($changebackcode)){
			$translate->setLocale($changebackcode);
    ...
    public function AddEditStatusCode(){
    	...
    	// echo "<pre>"; print_r(commonfunction::getTranslations(2));die;

    	// echo $this->ConvertTextbyLang('monday','fr_FR','en_US');die;
    	// echo "<pre>"; print_r($this->getData);print_r($translate); die;
		try{
    ...
    **/	 
	...
	public function GetMasterSmsTexts()
	{
		$select = $this->_db->select()->from(array('SM'=>SMS_TEXT),array('language_id','sms_text'))
		->where('SM.master_id=?',$this->getData['masterid']);
		$data = $this->getAdapter()->fetchAll($select);
		if(!empty($data)){

			foreach ($data as $key => $smstxt) {
				$res[] = array('language_id'=>$smstxt['language_id'],'smstxt'=>$smstxt['sms_text']);
			}
			return $res;
		}else return false;
	...
	public function AddMasterSmsTexts()
	{
		$error = false;
	   if(isset($this->getData['master_id']) && !empty($this->getData['master_id'])){
			...
			try {
			 	...
			 	$this->_db->delete(SMS_TEXT,'master_id='.$this->getData['master_id']);
					$languages = $this->SettingModelObj->ListLanguage();
					$languages = array_reduce($languages, function(&$reduced,$entry){
						$reduced[] = $entry['language_id'];
						return $reduced;
					});

					foreach ($this->getData as $key => $value) {
						if( in_array($key, $languages) )
							$postedLanguages[$key] = $value;
					}
					$postedLanguages = array_filter($postedLanguages);
					foreach ($postedLanguages as $key => $value) {
						$insertData = array(
							'master_id'=>$this->getData['master_id'],
							'language_id'=>$key,
							'sms_text'=>$value,
						);
						
						$res = $this->_db->insert(SMS_TEXT,$insertData);
						$error = $res? false: true; 
					}

				$resp = $error ? array('status'=>0,'message'=>'some internal error occurred') : array('status'=>1,'message'=>'Successfully Updated') ;
				return $resp;
